...
def search(request):
    ...
    if normalized_university:
        records = records.filter(course__uni__uni_name__iexact=university)
    if semester:
...
def upload(request):
    ...
    if request.method == 'POST':
        ...
        if not form.is_valid():
            # Grabs the first human-readable validation error generated by the form rules
        ...
        # --- EXPLICIT SAFE VALIDATION LOOP PLACEMENT ---
        # Replacing the unsafe 'all()' iteration to handle file streams cleanly
